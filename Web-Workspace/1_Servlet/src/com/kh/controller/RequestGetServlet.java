...
//		System.out.println("잘 실행되나?");
         ...
         * 
         * 첫 번째 매개변수인 HttpServletRequest request 에는 요청 시 전달된 내용들이 담김(사용자가 입력한 값),
         * 전송방식, 요청한 사용자의 ip등등
         * 
         * 두 번째 매개변수인 HttpServletResponse response 에는 요청 처리 후 응답할 때 사용하는 객체
         * 
         * 요청을 처리하기 위해 요청 시 전달된 값(사용자가 입력한 값)들을 뽑는다.
         * request의 parameter영역 안에 존재 => key-value 세트로 담겨있다. (name 속성값 = value값)
         * 
		...
		// 체크박스처럼 복수개의 정보를 받을 때는 배열 형태로 받아야함.
		...
		if(foods == null) {
			...
			// 배열에 있는 모든 값들을 구분자를 통해서 하나의 문자열로 반환해 주는 메소드.
		}
		
		// 뽑아낸 값들을 가지고 요청 처리해야함 (DB와 상호작용)
		// 일반적인 흐름 : Servlet -> Service(뽑아낸 값들을 넘기면서 비지니스 로직 실행) -> Dao호출 - DB SQL문 실행 - 결과값 반환
		
		// 위의 과정을 다 처리한 후 사용자가 보게 될 응답페이지를 만들어서 전달 해 주기
		
		/*
		 * 장점 : Java 코드 내에 작성하기 때문에 반복문, 조건문, 유용한 메소드를 사용할 수 있다.
		 * 단점 : 복잡, 혹시라도 나중에 html수정 해야 할 때 Java코드 내에서 수정이 이루어지기 때문에
		 * 		수정된 내용을 다시 반영시키고자 한다면 서버를 재실행 해야한다. 사소한 오타로 에러날 수 있다. 
		 */
		
		// * response 객체를 통해 사용자에게 html(응답화면)을 전달
		// 1) 이제부터 내가 출력 할 내용은 문서형태이고 html이다. 더불어서 문자셋은 utf-8 이라는것을 지정
		response.setContentType("text/html; charset=UTF-8");
		
		// 2) 응답하고자 하는 사용자(클라이언트)와의 스트림(클라이언트와의 통로)을 생성
		PrintWriter out = response.getWriter();
		
		// 3) 생성된 스트림을 통해 응답 html구문을 한 줄씩 출력
		out.println("<html>");
			out.println("<head>");
				out.println("<style>");
